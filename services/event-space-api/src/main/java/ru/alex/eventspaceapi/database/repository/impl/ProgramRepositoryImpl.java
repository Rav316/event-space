...
import ru.alex.eventspaceapi.database.repository.ProgramRepositoryCustom;
...
import static ru.alex.eventspaceapi.database.entity.QProgram.program;
...
public class ProgramRepositoryImpl implements ProgramRepositoryCustom {
    ...
    private static final String SQL_DELETE_IMPACT = """
            SELECT COUNT(*) AS users FROM users WHERE program_id = :id
            """;
    ...
    @Override
    public Page<Program> findAllByFilter(AdminListFilter filter, Sort sort) {
        ...
        OrderSpecifier<?>[] sortOrder = QueryDslUtils.toOrderSpecifiers(sort, SORT_BINDINGS, program.id.desc());
        ...
        return new PageImpl<>(programs, pageable, total != null ? total : 0);
    }
    ...
    public ProgramDeleteImpactDto getDeleteImpact(Integer id) {
        return jdbcTemplate.queryForObject(SQL_DELETE_IMPACT, Map.of("id", id), (rs, rowNum) ->
                new ProgramDeleteImpactDto(rs.getLong("users"))
        );
    }
