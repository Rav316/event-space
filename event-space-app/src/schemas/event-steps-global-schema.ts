...
export const eventStepsGlobalSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    for (let i = 1; i < steps.length; i++) {
      ...
      const [currStartH, currStartM] = steps[i].startTime.split(":").map(Number);
    ...
    const [lastEndH, lastEndM] = steps[steps.length - 1].endTime.split(":").map(Number);
    const lastEndMinutes = lastEndH * 60 + lastEndM;
    if (lastEndMinutes !== globalEndMinutes) {
      ctx.addIssue({
        code: "custom",
        message: "Последний шаг должен заканчиваться ровно во время окончания мероприятия",
