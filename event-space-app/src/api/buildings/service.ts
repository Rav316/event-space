import type {
  ...
  BuildingCreateDto,
  BuildingEditDto,
  BuildingDeleteImpactDto,
} from '@/api/buildings/model.ts';
...
export const existsByName = async (name: string): Promise<boolean> => {
  const response = await axiosInstance.get<boolean>(
    `${ApiRoutes.BUILDINGS}/exists-by-name`,
    {
      params: { name },
    },
  );
...
export const getDeleteImpact = async (
  id: number,
): Promise<BuildingDeleteImpactDto> => {
  const response = await axiosInstance.get<BuildingDeleteImpactDto>(
    `${ApiRoutes.BUILDINGS}/${id}/delete-impact`,
  );
  return response.data;
};
