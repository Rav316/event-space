...
import { EventCreateStepCircle } from '@/components/shared/event-create';
...
export const EventCreateStepper: React.FC<Props> = ({ currentStep }) => {
  const progress =
    eventCreateSteps.length > 0
      ? Math.round((currentStep / eventCreateSteps.length) * 100)
      : 0;
  return (
    <div className={'flex flex-col gap-2'}>
      <div className={'flex justify-between items-center w-full'}>
        <span>
          Шаг {currentStep + 1} из {eventCreateSteps.length}
        </span>
