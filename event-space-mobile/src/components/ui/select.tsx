      ...
      <SelectPrimitive.Root {...props}>
        {React.Children.map(children, (child) => {
          if (
            ...
            (child.type as any) === SelectTrigger
          ) {
            return React.cloneElement<any>(child, {
              onLayoutWidth: setTriggerWidth
            });
          }
          return child;
        })}
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'border-input dark:bg-input/30 dark:active:bg-input/50 bg-background flex h-10 flex-row items-center rounded-md border px-3 py-2 pr-8 relative shadow-sm shadow-black/5 sm:h-9',
        Platform.select({
            ...
            >
              <SelectPrimitive.Content
                className={cn(
                  ...
                  Platform.select({
                    web: cn(
                      'animate-in fade-in-0 zoom-in-95 origin-(--radix-select-content-transform-origin) max-h-52 overflow-y-auto overflow-x-hidden w-[var(--radix-select-trigger-width)]',
                  ...
                  className={cn(
                    ...
                    position === 'popper' &&
                      cn(
                        'w-full',
                        Platform.select({
                          web: 'h-[var(--radix-select-trigger-height)]'
                        })
                      )
                  )}
                >
